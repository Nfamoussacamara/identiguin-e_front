...
import { 
  ...
  ClipboardList, Link2, BarChart2, Settings
} from 'lucide-react';
...
const DashboardLayout = () => {
  ...
  const adminMenuItems = [
    ...
    { name: 'Demandes',        icon: <ClipboardList size={20} />,   path: '/admin/demandes' },
    { name: 'Vérifications',   icon: <ShieldCheck size={20} />,     path: '/admin/verifications' },
    { name: 'Blockchain',      icon: <Link2 size={20} />,           path: '/admin/blockchain' },
    { name: 'Rapports',        icon: <BarChart2 size={20} />,       path: '/admin/rapports' },
    ...
    <div className="dashboard-theme flex flex-col h-screen overflow-hidden font-sans bg-[#F8F7F7]">
      ...
      <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-4 md:px-8 z-30 shrink-0 shadow-sm">
        ...
        <div className="flex items-center space-x-3 md:space-x-6 text-gray-500">
          ...
          <button className="flex items-center space-x-2 md:space-x-3 bg-[#009A44] hover:bg-[#008A3D] text-white px-3 md:px-4 py-2.5 rounded-xl shadow-sm transition-all">
            {isLoading ? (
              <div className="flex items-center space-x-2">
                <Skeleton variant="circle" className="w-8 h-8 opacity-50" />
                <Skeleton className="h-3 w-20 hidden md:block opacity-50" />
              </div>
            ) : (
              <>
                <div className="w-8 h-8 bg-[#E2E9F3] text-slate-700 rounded-full flex items-center justify-center shrink-0">
                  <User size={18} strokeWidth={2.5} />
                </div>
                <span className="text-xs md:text-sm font-bold tracking-wide truncate max-w-[80px] md:max-w-none">
                  {profile?.nom_complet || (isAdmin ? 'Admin Système' : 'Citoyen')}
                </span>
                <ChevronDown size={16} className="shrink-0 text-white" strokeWidth={3} />
              </>
            )}
          </button>
        </div>
      ...
      <div className="flex flex-grow overflow-hidden relative">
        ...
        >
          <nav className="flex-grow mt-6 px-3 space-y-1 min-w-[256px]">
            ...
            {/* Suivi en direct (citoyen uniquement) */}
            {!isAdmin && (
        ...
        <main className="flex-grow overflow-y-auto p-4 md:p-8 flex flex-col">
          ...
          <footer className="mt-12 pt-6 border-t border-gray-100 flex flex-col md:flex-row gap-4 justify-between items-center text-[10px] text-gray-400 font-medium uppercase tracking-widest text-center">
        ...
        {/* ── FLOATING PILL (citoyen uniquement) ───────────────────── */}
        {!isAdmin && (
      ...
      {/* Modal Nouvelle Demande (citoyen uniquement) */}
      {!isAdmin && (
        <NewRequestModal
